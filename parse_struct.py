...
class Parse:
    def __init__(self, fn_datatype, fn_datastruct, locale='utf-8'):
        ...
        self.locale = locale
        #f=open(fn_datatype)
        f=open(fn_datatype,encoding=locale)  #0.04
        self.file_datatype = f.readlines()
        f.close()

        #f=open(fn_datastruct)
        f=open(fn_datastruct,encoding=locale) #0.04
